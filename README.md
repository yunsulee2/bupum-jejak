...
제품마다 다른 문제 해결 과정을 3D로 연습하는 실습형 프로토타입입니다. 현재 메인 화면에서 `게이밍 컴퓨터 조립`과 `형광등 교체 진단`을 선택할 수 있습니다.
...
현재 버전은 실무 교육을 대체하거나 자격을 인증하지 않는 훈련 보조 프로토타입입니다. 형광등 모듈은 램프 자체 교체만 다루며 소켓·안정기·배선 수리나 LED 대체관 개조를 안내하지 않습니다. 유리관 파손, 그을린 소켓, 안정기 타는 냄새가 있으면 작업을 멈추고 전기 전문가와 관할 지자체의 폐기 지침을 확인해야 합니다. 다음 단계에서 실제 조립·전기 전문가 검수를 거쳐 케이블 규격과 실패 진단 시나리오를 제품군별로 분리해야 합니다.
