...
제품마다 다른 문제 해결 과정을 3D로 연습하는 실습형 프로토타입입니다. 현재 메인 화면에서 `게이밍 컴퓨터 조립`, `형광등 교체 진단`, `샤워기 필터 교체`, `3단 서랍장 조립`을 선택할 수 있습니다.
...
- 데스크톱·모바일 반응형 UI
- 선택 전에는 3D 엔진을 내려받지 않는 실습별 지연 로딩과 3 kB대 첫 화면 진입 청크
- 4K 데스크톱 모델(약 6.3 MB)과 2K 모바일 모델(약 3.2 MB)을 자동 선택하는 Meshopt 자산 파이프라인
- 탭이 보이지 않을 때 렌더 루프를 멈추고 모바일 픽셀 비율·그림자 해상도를 조절하는 균형형 렌더링
...
- 판재 안팎·캠락 방향·좌우 레일·뒤판 직각·전도 방지를 차례로 검증하는 3단 서랍장 조립
...
`npm run verify`는 프로덕션 빌드, 청크·3D 모델·로비 이미지 성능 예산, 21개 기능 테스트를 한 번에 실행합니다.
...
- 모바일 출력: `public/models/pc-lab-mobile.glb`
- 생성기: `tools/build_pc_scene.py`

Blender 출력 뒤에는 `npm run assets:optimize`가 노드 이름을 유지한 채 Meshopt 압축을 적용하고, 모바일용 2K 텍스처 모델을 함께 만듭니다.
