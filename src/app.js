...
let pcDataPromise = null;
let pcModelPromise = null;
...
function renderCart() {
  ...
  ui.purchase.disabled = selectedCount !== state.shopCategories.length || !state.ready;
  ui.purchase.setAttribute('aria-busy', String(!state.ready));
}
...
async function startExperience() {
  setModuleLoadState(ui.start, 'loading', '구매 목록 준비 중…');
  try {
    await ensurePcData();
  } catch (error) {
    console.error(error);
    setModuleLoadState(ui.start, 'error', '구매 목록 오류 · 다시 누르세요');
    return;
  }
  setModuleLoadState(ui.start, 'ready');
  openShop();
  ensurePcModel().catch((error) => {
    console.error(error);
    showToast('3D 조립실을 준비하지 못했습니다. 상태 문구를 눌러 다시 시도해 주세요.', 'error');
  });
}

function setModuleLoadState(button, loadState, label = '') {
  const status = button.querySelector('.module-card-copy small');
  if (!button.dataset.originalStatus) button.dataset.originalStatus = button.dataset.bootstrapStatus || status.textContent;
...
async function ensurePcData() {
  if (state.data && state.catalog) return;
  if (pcDataPromise) return pcDataPromise;
  pcDataPromise = Promise.all([
...
function setPcModelStatus(status, label) {
  ui.pcModelStatus.dataset.state = status;
  ui.pcModelStatus.textContent = label;
  ui.pcModelStatus.disabled = status !== 'error';
}

async function ensurePcModel() {
  if (state.ready) return modelScene;
  if (pcModelPromise) return pcModelPromise;
  setPcModelStatus('loading', `${renderProfile.balanced ? '균형형' : '고화질'} 3D 조립실 준비 중…`);

  pcModelPromise = new Promise((resolve, reject) => {
    loader.load(
      renderProfile.modelUrl,
      (gltf) => {
        ...
        initializePartLayout();
        state.ready = true;
        setPcModelStatus('ready', `${renderProfile.balanced ? '균형형' : '고화질'} 3D 조립실 준비 완료`);
        renderCart();
        if (!ui.shop.hidden) showToast('3D 조립실 준비가 끝났습니다.', 'success');
        resolve(modelScene);
      },
      (event) => {
        const progress = event.total ? event.loaded / event.total : 0.55;
        setPcModelStatus('loading', `${renderProfile.balanced ? '균형형' : '고화질'} 3D 조립실 ${Math.round(Math.min(1, progress) * 100)}%`);
      },
      (error) => {
        pcModelPromise = null;
        state.ready = false;
        setPcModelStatus('error', '3D 조립실 준비 실패 · 눌러서 다시 시도');
        renderCart();
        reject(error);
      },
    );
  });
  return pcModelPromise;
}
...
function animate(now) {
  ...
  if (!document.hidden) animationFrameId = window.requestAnimationFrame(animate);
}
...
ui.pcModelStatus.addEventListener('click', () => {
  ensurePcModel().catch((error) => {
    console.error(error);
    showToast('3D 조립실을 다시 준비하지 못했습니다. 네트워크 연결을 확인해 주세요.', 'error');
  });
});
...
export async function launchExperience(kind) {
  if (kind === 'pc') return startExperience();
  if (kind === 'fluorescent') return startFluorescentExperience();
  if (kind === 'shower') return startShowerFilterExperience();
  if (kind === 'drawer') return startDrawerExperience();
  throw new Error(`지원하지 않는 실습 모듈입니다: ${kind}`);
}
